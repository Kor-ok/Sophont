...
def _type_parser(input_value: Any) -> str:
    ...
    for type_arg in (type_args if isinstance(type_args, tuple) else (type_args,)):
        ...
        break
    return f"{Fore.RESET}{input_value!r} {colour}{type_args}{Style.RESET_ALL}"
...
converters: ConvertersType = {
    ...
    "tuple[int, …]": Converters.tuple_ints,
    "nested tuple[Any, …]": Converters.tuple_ints,
...
def debug_display() -> None:
    ...
    def test_fake_authored_data_cases() -> None:
        ...
        for case_name, input_value in result.items():
            print(f"{Fore.CYAN}{case_name}: {(' ' * (alignment_padding - len(case_name)))}{_type_parser(input_value)}{Style.RESET_ALL}")

    def test_converted_values() -> None:
        for case_name, input_value in result.items():
            alignment_padding = values_aligner(list(result.keys()))
            print(f"{Fore.YELLOW}{case_name}: {(' ' * (alignment_padding - len(case_name)))}{_type_parser(input_value)}{Style.RESET_ALL}")
            value = input_value.value if hasattr(input_value, "value") else f"{Fore.RED}?{input_value}{Style.RESET_ALL}"
            print(f"{Fore.BLUE}    Extracted: {value}{Style.RESET_ALL}")
            # convert_authored_data_via_ast(input_value)
            # try:
            #     converted, type_arg = convert_authored_data_via_ast(input_value)
            # except Exception as e:
            #     print(f"{Fore.RED}Error converting {case_name}: {e}{Style.RESET_ALL}")
            #     continue
            # alignment_padding = values_aligner(list(result.keys()))
            # print(f"{Fore.MAGENTA}Converted {case_name}: {(' ' * (alignment_padding - len(case_name)))}{_type_parser(converted)} {Fore.BLUE}({type_arg}){Style.RESET_ALL}")
