...
BySignature: TypeAlias = dict[Signature, FlattenedAliasMap] # Creating a single Signature tuple by adding the domain identity at the start.
"""Forward index: (DomainIdentity, Signature) → FlattenedAliasMap."""

ByAliasForSignature: TypeAlias = dict[tuple[DomainIdentity, FlattenedAliasMap], Signature]
"""Reverse index: (DomainIdentity, FlattenedAliasMap) → Signature."""

ByMemberIdentity: TypeAlias = dict[tuple[DomainIdentity, MemberIdentity, Signature], FlattenedAliasMap]
"""Member index: (DomainIdentity, MemberIdentity, Signature) → FlattenedAliasMap."""

ByAliasForMemberIdentity: TypeAlias = dict[tuple[DomainIdentity, MemberIdentity, FlattenedAliasMap], Signature]
...
def get_semantics_from_instance(instance: Any, definitions: DefinitionsIndices) -> None:
    ...
    base_class = instance.__class__.mro()[-2]
    ...
    if subset_count > 1:
        ...
        for involved_class, involved_signature in involved_classes_with_signatures.items():
            # Use the first element of the involved signature as the domain identity to look up the 
            # class name in the subclass dict, and then use the class name to look up the semantics 
            # in the by_signature index. We also need to convert the involved signature to a tuple 
            # of ints if it's not already, to match the format of the signatures in the by_signature index.
        ...
        print(f"Semantics for class {list(composite_signatures.keys())[0]} with signature {first_combined_composite_signature}: {Fore.GREEN}{semantics}{Style.RESET_ALL}")
        ...
        try:
            ...
            semantics = "Semantics not found for this signature"
        print(f"Semantics for class {list(composite_signatures.keys())[1]} with signature {second_composite_signature}: {Fore.GREEN}{semantics}{Style.RESET_ALL}")
    else:
